...
class AgainstWb5Worker(mp.Process):
    ...
    def run(self) -> None:
        ...
        trained_model_locker = rl_cpp.ModelLocker(
            [torch.jit.script(VecEnvAgent(trained_net)).to(self._device)], self._device)
        if self._search_config["use_sl_net_as_opponent"]:
            ...
            sl_model_locker = rl_cpp.ModelLocker(
                [torch.jit.script(VecEnvAgent(supervised_net)).to(self._device)], self._device)
            actors_ns = [rl_cpp.VecEnvActor(trained_model_locker), rl_cpp.VecEnvActor(sl_model_locker),
                         rl_cpp.VecEnvActor(trained_model_locker), rl_cpp.VecEnvActor(sl_model_locker)]
            actors_ew = [rl_cpp.VecEnvActor(sl_model_locker), rl_cpp.VecEnvActor(trained_model_locker),
                         rl_cpp.VecEnvActor(sl_model_locker), rl_cpp.VecEnvActor(trained_model_locker)]
        else:
            actors_ns = [rl_cpp.VecEnvActor(trained_model_locker) for _ in range(NUM_PLAYERS)]
            actors_ew = [rl_cpp.VecEnvActor(trained_model_locker) for _ in range(NUM_PLAYERS)]
        net = PolicyNet()
        ...
        params.verbose_level = int(self._process_id == 0)
        params.seed = self._search_config["seed"]
        params.select_highest_rollout_value = True
        searcher_ns = rl_cpp.Searcher(params, actors_ns, 1000)
        searcher_ew = rl_cpp.Searcher(params, actors_ew, 1000)
        i_deal = 0
        _imps = []
            ...
            try:
                ...
                while not state_0.terminated():
                    ...
                    if current_player % 2 == 0:
                        ...
                        action = searcher_ns.search(state_0, probs)
                        # print(action)
                    else:
                ...
                while not state_1.terminated():
                    ...
                    if current_player % 2 == 1:
                        ...
                        sum_probs = probs.sum()
                        # print("probs: ", probs)
                        action = searcher_ew.search(state_1, probs)
