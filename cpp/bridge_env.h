...
torch::Tensor MakeObsTensor(const std::shared_ptr<BridgeBiddingState> &state, int greedy) {
    torch::Tensor obs = torch::zeros(kObsTensorSize, {torch::kFloat});
    auto observation = state->ObservationTensor();
    obs.slice(0, 0, kAuctionTensorSize).copy_(torch::tensor(torch::ArrayRef<float>(observation)));
    auto legal_actions = state->LegalActions();
    auto legal_actions_mask = LegalActionsMask(legal_actions);
    obs.slice(0, kAuctionTensorSize, kAuctionTensorSize + kNumCalls).copy_(legal_actions_mask);
    obs[-1] = greedy;
...
public:
    ...
    torch::Tensor Reset() override {
        num_states_ += 1;
//        std::cout << card_trajectories_ << std::endl;
        current_cards_ = card_trajectories_[cursor_];
//        std::cout<< current_cards_ << std::endl;
        current_ddt_ = ddts_[cursor_];
//        std::cout << current_ddt_ << std::endl;
        cursor_ = (cursor_ + 1) % size_;


        state_ = std::make_shared<BridgeBiddingState>(dealer_, current_cards_, false,
                                                      false, current_ddt_);
    ...
    std::tuple<torch::Tensor, float, bool> Step(const torch::Tensor &action) override {
    ...
    std::tuple<std::vector<Action>, std::vector<int>> GetCurrentCardsAndDDT() const {
    ...
    std::shared_ptr<BridgeBiddingEnv> MakeSubEnv() {
        ...
        return std::make_shared<BridgeBiddingEnv>(cards_trajectories, ddts, greedy_);
...
public:
    ...
    torch::Tensor Reset() override {
        ...
        cursor_ = (cursor_ + 1) % size_;
        num_states_++;
        auto state = std::make_shared<BridgeBiddingState>(0, current_cards_,
                                                          false, false, current_ddt_);
        states_[0] = state;
        current_state_ = 0;
    ...
    std::tuple<torch::Tensor, float, bool> Step(const torch::Tensor &action) override {
        ...
        if (current_state_ == 0 && states_[0]->Terminated()) {
            current_state_ = 1;
            states_[1] = std::make_shared<BridgeBiddingState>(0, current_cards_,
                                                              false, false, current_ddt_);
            obs_tensor = MakeObsTensor(states_[1], greedy_[ActingPlayer()]);
        } else {
            obs_tensor = MakeObsTensor(states_[current_state_], greedy_[ActingPlayer()]);
        }
        float r = 0.0f;
        bool t = Terminated();
    ...
    std::vector<int> HistoryImps() const{
        return history_imps_;
    }
