...
public:
    ...
    void PushObsAndActionAndLogProbs(const torch::Tensor &obs, const torch::Tensor &action,
    ...
    void PostToReplayBuffer(const std::shared_ptr<ReplayBuffer> &buffer, float final_reward) {
//        std::vector<float> cum_reward = ComputeCumulativeRewards(final_reward);
