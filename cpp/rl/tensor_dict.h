...
inline TensorDict IndexSelect(const TensorDict &batch, const torch::Tensor &indices) {
  TensorDict result;
  for (const auto &name2tensor : batch) {
    result.insert({name2tensor.first, name2tensor.second.index_select(0, indices)});
  }
  return result;
}
...
// stack 2 batches
inline TensorDict BatchStack(const TensorDict &batch1, const TensorDict &batch2){
  TensorDict ret;
  for (auto &name2tensor : batch1) {
    if(batch1.at(name2tensor.first).ndimension()==1){
      ret[name2tensor.first] = torch::hstack({batch1.at(name2tensor.first), batch2.at(name2tensor.first)});
    }else{
      ret[name2tensor.first] = torch::vstack({batch1.at(name2tensor.first), batch2.at(name2tensor.first)});
    }
  }
  return ret;
}
...
inline void Print(const TensorDict &batch) {
  for (const auto &name2tensor : batch) {
    std::cout << name2tensor.first << ":\n"<< name2tensor.second << std::endl;
  }
}
